...
from .. import db
from ..models import Permission, User, Post, Comment, Notification, Like, Transaction, Activity, Collect, Want, Role
...
def index():
    ...
    if request.method == 'GET':
        ...
        if type is not None and type is not "":
            query1 = Post.query.filter_by(type=type)
        ...
        for item in query1:
            ...
            time = datetime.utcnow() - item.timestamp
            if time < time_list[0]:
                weight = 10
                ...
                weight = 3
            elif time < time_list[3]:
                weight = 1
            else:
                weight = 0
            item.important = 7 * com_num + 3 * li_num + weight
        hot = query1.order_by(Post.important.desc()).all()
        ...
        hot_activity = li.order_by(Activity.important.desc()).all()
        return render_template('index/index_posts.html', posts1=posts1, posts5=hot,
                               pagination1=pagination1, hot_activity=hot_activity, flag=1)
    # ’POST‘方法
    else:
...
def index_transaction():
    if request.method == 'GET':
        ...
        return render_template('index/index_transactions.html', transactions=transactions, posts5=hot,
                               hot_activity=hot_activity, flag=2)
    else:
        inf = request.form["search"]
        return redirect(url_for('.query', content=inf))
    ...
    if request.method == 'GET':
        ...
        for activity in query3:
            if activity.activity_time < datetime.utcnow():
                ...
                db.session.commit()
        activities = query3.order_by(Activity.timestamp.desc()).all()
        # hot
        query1 = Post.query
        for item in query1:
            item.important = 0
        ...
        return render_template('index/index_activities.html', activities=activities, posts5=hot,
                               hot_activity=hot_activity, flag=3)
    else:
        inf = request.form["search"]
        return redirect(url_for('.query', content=inf))
...
def index_follow():
    if request.method == 'GET':
        ...
        for item in li:
            ...
            item.important = li_num
        hot_activity = li.order_by(Activity.important.desc())
        return render_template('index/index_follows.html', posts4=posts4, posts5=hot, hot_activity=hot_activity, flag=4)
    else:
        inf = request.form["search"]
        return redirect(url_for('.query', content=inf))
...
def query_user():
    ...
    if request.method == 'POST':
        ...
        if inf == "":
            ...
            return render_template('queryuser.html')
        search_result = "%" + inf + "%"
        result = User.query.filter(or_(User.username.like(search_result), User.email == search_result))
        page = request.args.get('page', 1, type=int)
...
def query_transaction():
    ...
    if request.method == 'POST':
        ...
        if inf == "":
            ...
            return render_template('querytransaction.html')
        search_result = "%" + inf + "%"
        result = Transaction.query.filter(or_(Transaction.car_name.like(search_result),
                                              Transaction.car_describe.like(search_result)))
        page = request.args.get('page', 1, type=int)
...
def edit_profile():
    ...
    if request.method == 'POST':
        ...
        current_user.car = request.form["car"]
...
def like(post_id):
    ...
    count = int(data.get('num'))
    print(data.get('status'))
    print(data.get('num'))
    print("------")
    print(post_id)
    post = Post.query.filter_by(id=post_id).first()
    print(current_user.is_liking(post))
    current_user.like(post)
    count += 1
    post.like(current_user)
    post.recent_activity = datetime.utcnow()
    db.session.commit()
    return jsonify({'result': count, 'alert': 'success'})
    flash('You are now liking this post')
    return redirect(url_for('.index', id=post_id))


@main.route('/likeinpost/<post_id>', methods=['GET', 'POST'])
@login_required
@permission_required(Permission.FOLLOW)
...
# 取消对帖子（问题）点赞的功能
@main.route('/dislike/<post_id>', methods=['GET', 'POST'])
@login_required
@permission_required(Permission.FOLLOW)
def dislike(post_id):
    print("删除id" + post_id)
...
@main.route('/dislikeinpost/<post_id>', methods=['GET', 'POST'])
@login_required
@permission_required(Permission.FOLLOW)
