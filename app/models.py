...
class Role(db.Model):
    ...
    def insert_roles():
        roles = {
            ...
            'Company': [Permission.FOLLOW, Permission.COMMENT,
...
class User(UserMixin, db.Model):
    ...
    car = db.Column(db.String(128), default="")
    ...
    def collect(self, transaction):
        if not self.is_collecting(transaction):
            ...
            n = Notification(receiver_id=transaction.seller_id, timestamp=datetime.utcnow(),
                             ...
                             object=transaction.car_name, object_id=transaction.id)
            db.session.add(n)
            db.session.add(ll)
...
class Transaction(db.Model):
    ...
    car_describe = db.Column(db.Text, nullable=False)
