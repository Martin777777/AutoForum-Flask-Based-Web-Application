...
def register():
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        try:
            register_cloud(0, request.form["email"], request.form["user_name"], request.form["confirm_pwd"], None, '/static/assets/qq.jpg')
        except:
            pass
        # 注册时发送邮箱认证
...
def confirm(token):
    ...
    if current_user.confirm(token):
        db.session.commit()
        try:
            confirm_cloud(current_user.email)
        except:
            pass
...
def password_reset(token):
    ...
    if form.validate_on_submit():
        if User.reset_password(token, form.password.data):
            db.session.commit()
            try:
                reset_password_cloud(form.password.data, current_user.email)
            except:
                pass
...
def change_password():
    ...
    if request.method == 'POST':
        ...
        if current_user.verify_password(old_password):
            ...
            db.session.commit()
            try:
                change_password_cloud(password, current_user.email)
            except:
                pass
            flash('Your password has been updated.')
            return redirect(url_for('main.index'))
...
def change_email(token):
    if current_user.change_email(token):
        ...
        try:
            change_email_cloud(new_email, current_user.email)
        except:
            pass
        db.session.commit()
